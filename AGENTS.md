...
AICMF is designed to be administered and developed by AI agents. This file is
the contract for how that work is done. `CLAUDE.md` is a symlink to this file.
...
- `Makefile` wraps the deploy flow (`make update`, `make test`) and works with
  Docker or Podman.
...
- **Content** (`content/`): Markdown + YAML frontmatter
  (`title`, `date`, `description`, `tags`). The filesystem is the editor — to
  publish, drop a file in `content/articles/` and run `app:sync`. Frontmatter
  `description` doubles as the SEO meta description.
...
- **HTTP** (`src/Controller/`): `WebController` (`/`, `/article/{slug}`),
  `SearchController` (`GET /api/search?q=` — hybrid semantic/FTS),
  `SitemapController` (`/sitemap.xml`, rendered live from the index).
- **Theme** (`src/Themes/default/`): Twig templates. `base.html.twig` exposes
  `title` and `description` blocks; the `asset_v()` Twig function
  (`src/Twig/AssetExtension.php`) appends an mtime query string for
...
- **The slug is the file path**, not the title: relative path with `/` replaced
  ...
  → URL `/article/articles-my-post`. Renaming or moving a file changes its URL.
...
subdirectory (see TDD-first above) — content changes don't need tests.

## Content authoring rules

When generating or editing articles in `content/`, the text must be **clean,
plain human text** — no artifacts that mark it as AI-generated. Such markers can
get pages flagged by AI-content detectors and ruin SEO ranking.
...
- **Write like a person:** natural sentence rhythm and varied structure; the
  frontmatter `description` doubles as the SEO meta description, so it must
  read naturally too.
...
- `.env.local` (git-ignored) is the **single source of truth** on a server —
  copy it from `.env.local.example`.
...
- `SITE_BASE_URL` must be the public https origin — behind a TLS proxy the
  container only sees plain HTTP, so the request scheme can't be trusted for
  generated absolute URLs (sitemap).
...
- [hcnotes](https://github.com/matasarei/hcnotes) — a real production site built
  on AICMF; a good example of how a downstream project layers a theme and
  content on top of this framework.
